...
class AdjudicationPipelineOrchestrator:
    ...
    def __init__(
        self,
        coverage_matcher, exclusion_analyst, adjudication_drafter,
    ...
    def _degrade_to_plain_rag(self, claim_id: str, job_id: str) -> PipelineResult:
        ...
        return PipelineResult(
            job_id=job_id,
            steps=[{"agent": "plain_rag_fallback", "result": result.output}],
            final_recommendation={
                "note": "Full agent pipeline unavailable - degraded to direct policy search, still filtered to the correct policy version where resolvable. Escalate for manual review.",
