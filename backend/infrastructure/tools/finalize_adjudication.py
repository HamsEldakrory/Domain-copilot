...
class FinalizeAdjudicationTool(Tool):
    ...
    def run(self, claim_id: str, approved_by: str | None, outcome: str, rationale: str) -> ToolResult:
        if not approved_by:
            return ToolResult(
                tool_name=self.name,
                error="finalize_adjudication cannot run without approved_by - enforced pending the full approval gate (Day 6).",
            )
        try:
        ...
        decision = Decision.objects.create(claim=claim, approved_by_id=approved_by, outcome=outcome, rationale=rationale)
